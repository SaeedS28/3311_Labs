...
feature -- Events
	...
	move_left (r, c: INTEGER)
		...
		do
			...
			else if not game.board.is_valid_column (c-1) then
					message := error_column_not_valid(c-1)
			else if not game.board.is_valid_column (c-2) then
		...
		do
			if not game.board.is_valid_row (r) then
				message := error_row_not_valid(r)
					...
					message := error_column_not_valid(c)
			else if not game.board.is_valid_row (r-1) then
					message := error_row_not_valid(r-1)
			else if not game.board.is_valid_row (r-2) then
